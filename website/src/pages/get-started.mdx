...
All code through the education, as well as the each of the examples, is designed to be read and understood. To achieve this, it is kept simple and readable. Every bit that needs context will contain comments with in-depth explanations.

Please read each of the code blocks throughout the education with care.
...
```graphql filename="examples/schema/basic/schema.graphql"
type Query {
  """
...
```graphql filename="examples/schema/subscriptions/schema.graphql"
type Subscription {
  taskCreated: Task!
...
```graphql filename="examples/schema/authentication/schema.graphql"
extend type Query {
  """
...
```graphql filename="examples/schema/authorization/schema.graphql"
extend type Task {
  """
  Private tasks can be viewed and modified only by the user who created it.
  """
  private: Boolean!
}
```
